...
// ── Duplicate detection ───────────────────────────────────────────────────────
async function isDuplicate(jobUrl) {
...
async function ensureLoggedIn(page) {
    ...
    const signedIn = (await page.locator('[aria-label*="account"], [data-gnav-element-name="SignOut"], #SignedInLink').count()) > 0;
    if (signedIn) {
        console.error("[indeedApply] Session active — already logged in");
        return true;
    }
    console.error("[indeedApply] Not logged in. Opening Indeed sign-in page...");
    console.error("[indeedApply] Waiting up to 2 minutes for manual login. Sign in to Indeed in the browser window.");
    await page.goto("https://secure.indeed.com/auth", {
        waitUntil: "domcontentloaded",
    });
    try {
        await page.waitForURL((u) => u.href.includes("indeed.com") &&
            !u.href.includes("/auth") &&
            !u.href.includes("/login"), { timeout: 120000 });
        ...
        console.error("[indeedApply] Login not confirmed — will attempt applying without logged-in session");
...
function buildSearchUrl(filters) {
    ...
    if (filters.indeed_apply_only)
        params.set("iafc", "1"); // Indeed Apply only
...
async function scrapeJobs(page, searchUrl, maxJobs, indeedApplyOnly) {
    ...
    for (const card of cards.slice(0, maxJobs * 2)) {
        try {
            // Title
            const titleEl = card.locator("h2.jobTitle a, .jcs-JobTitle, a[data-jk]").first();
            const title = ((await titleEl.textContent().catch(() => "")) || "").trim();
            if (!title)
                continue;
            // Job key (used to build the job URL)
            const jobKey = (await titleEl.getAttribute("data-jk").catch(() => "")) ||
            ...
            // "Easily apply" badge
            const hasIndeedApply = (await card.locator(':text("Easily apply"), .indeedApplyBadge').count()) > 0;
            if (indeedApplyOnly && !hasIndeedApply)
                continue;
            jobs.push({
                id,
                title,
...
async function applyIndeedJob(page, job, profile, coverLetter, dryRun) {
    ...
    try {
        ...
        const applyBtn = page.locator('button:has-text("Apply now"), button:has-text("Easily apply"), #indeedApplyButton, .indeed-apply-button');
        ...
        while (stepCount < maxSteps) {
            ...
            const submitBtn = page.locator('button:has-text("Submit your application"), button:has-text("Submit application"), button[type="submit"]:has-text("Submit")');
            ...
            const nextBtn = page.locator('button:has-text("Continue"), button:has-text("Next"), button[type="submit"]:not(:has-text("Submit"))');
            ...
            // No navigation found
            console.error(`[indeedApply] No navigation button at step ${stepCount} for ${job.title}`);
            break;
        }
        const screenshotPath = await takeScreenshot(page, job.company);
...
export async function indeedApply(args) {
    const { candidate_profile, role, location = "Remote", remote = true, indeed_apply_only = true, date_posted_days = 7, job_type = "full_time", salary_min, min_fit_score = 60, max_applications = 10, tone = "professional", dry_run = false, } = args;
    ...
    try {
        ...
        const qualifying = scored
            ...
            .slice(0, maxApps);
        console.error(`[indeedApply] ${scored.length} scored, ${qualifying.length} qualify (score >= ${min_fit_score})`);
        ...
        for (const { job, score } of qualifying) {
            ...
            const notionStatus = result.success ? "Applied" : "Pending";
            const notionNotes = [
                `[indeed_apply]`,
            ...
            content: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        confirmation_rate: totalApplied > 0
...
//# sourceMappingURL=indeedApply.js.map
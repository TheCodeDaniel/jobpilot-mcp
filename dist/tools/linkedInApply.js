...
function buildSearchUrl(filters) {
    ...
    if (jtCodes.length > 0)
        params.set("f_JT", jtCodes.join(","));
    return `https://www.linkedin.com/jobs/search/?${params.toString()}`;
}
// ── Scrape job listings from the search results page ─────────────────────────
async function scrapeJobs(page, searchUrl, maxJobs) {
    ...
    for (const card of cards.slice(0, maxJobs * 2)) {
        try {
            // Title and URL
            const titleLink = card.locator("a.job-card-list__title--link, a.job-card-container__link, .job-card-list__title a").first();
            ...
            const company = ((await card
                .locator(".job-card-container__primary-description, .job-card-container__company-name")
                .first()
                .textContent()
...
async function applyEasyApply(page, job, profile, coverLetter, dryRun) {
    ...
    try {
        ...
        while (stepCount < maxSteps) {
            ...
            const modalSelector = ".jobs-easy-apply-modal, [data-test-modal], .artdeco-modal";
            ...
            const textareas = page.locator(`${modalSelector} textarea:visible, .jobs-easy-apply-form-section textarea:visible`);
            ...
            const submitBtn = page.locator('button:has-text("Submit application"), button[aria-label="Submit application"]');
            ...
            const reviewBtn = page.locator('button:has-text("Review your application"), button:has-text("Review")');
            ...
            const nextBtn = page.locator('button:has-text("Next"), button[aria-label="Continue to next step"]');
            ...
            console.error(`[linkedInApply] No navigation button at step ${stepCount} for ${job.title} — stopping`);
            break;
        }
        const screenshotPath = await takeScreenshot(page, job.company);
...
export async function linkedInApply(args) {
    const { candidate_profile, role, location = "Worldwide", remote = true, easy_apply_only = true, date_posted = "week", experience_levels = ["entry", "associate", "mid_senior"], job_types = ["full_time"], min_fit_score = 60, max_applications = 10, tone = "professional", dry_run = false, } = args;
    ...
    try {
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            success: false,
                            message: "LinkedIn login timed out. Run linkedin_apply again and log in within 2 minutes. Your session will be saved for all future runs.",
                            total_found: 0,
                            total_applied: 0,
                            total_confirmed: 0,
                            applications: [],
        ...
        const qualifying = scored
            ...
            .slice(0, maxApps);
        console.error(`[linkedInApply] ${scored.length} scored, ${qualifying.length} qualify (score >= ${min_fit_score})`);
...
//# sourceMappingURL=linkedInApply.js.map
...
function extractJobKeywords(job) {
    ...
    const multiWord = text.match(/(?:react native|vue\.?js|node\.?js|next\.?js|ruby on rails|machine learning|deep learning|ci\/cd|rest api|graphql api|data engineering|cloud computing|project management|product management|ui\/ux|ux design|web development|mobile development|full[ -]?stack|front[ -]?end|back[ -]?end)/gi);
...
export async function scoreJobFit(args) {
    ...
    const candidateTitle = normalize((candidate_profile?.experience || []).map((e) => e.title).join(" ") || "");
    ...
    const missing_skills = techTerms.filter((tag) => tag.length > 1 && !candidateSkills.some((s) => s.includes(tag) || tag.includes(s)));
    // ── Scoring ────────────────────────────────────────────────────────────────
    let score = 0;
    ...
    const hasPrimaryMatch = jobTitleWords.some((tw) => candidateSkills.some((s) => s.includes(tw) || tw.includes(s)));
    if (hasPrimaryMatch) {
    ...
    // Leadership bonus (+5)
    const hasLeadership = (candidate_profile?.experience || []).some((e) => /\b(lead|senior|principal|staff|head|director|manager)\b/i.test(e.title));
    if (hasLeadership) {
        score += 5;
    }

...
async function applyToJob(page, job, profile, coverLetter, dryRun) {
    ...
    try {
        ...
        const easyApplyBtn = page.locator('button:text-matches("Easy Apply|Quick Apply", "i"), a:text-matches("Easy Apply|Quick Apply", "i")');
        ...
        for (const [atsName, config] of Object.entries(ATS_CONFIGS)) {
            if (config.detect(currentUrl)) {
                ...
                if (sel.first_name)
                    await fillField(page, sel.first_name, firstName);
                if (sel.last_name)
                    await fillField(page, sel.last_name, lastName);
                if (sel.name)
                    await fillField(page, sel.name, profile.name);
                await fillField(page, sel.email, profile.email);
                if (sel.phone && profile.phone)
                    await fillField(page, sel.phone, profile.phone);
        ...
        const applyBtn = page.locator('a:text-matches("Apply", "i"), button:text-matches("Apply", "i")');
...
async function fillByLabel(page, labelTerms, value) {
    if (!value)
        return;
    try {
...
async function takeScreenshot(page, company) {
    try {
...
export async function autoApply(args) {
    const { candidate_profile, role, max_applications = 5, min_fit_score = 65, tone = "professional", dry_run = false, } = args;
    ...
    const foundJobs = searchData.jobs;

...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            ...
            {
                name: "linkedin_apply",
                description: "Search LinkedIn for jobs and auto-apply using Easy Apply. Uses a persistent browser session — on first run the user logs in manually; all subsequent runs reuse the saved session. Each application result includes a 'confirmed' flag, confidence level, confirmation message, and a screenshot path so you can verify every submission.",
                inputSchema: {
                    type: "object",
                    properties: {
                        candidate_profile: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case "linkedin_apply":
                return await linkedInApply(args);
            case "indeed_apply":
                return await indeedApply(args);
            default:

...
export async function generateCoverLetter(args: {
  ...
  if (tone === "enthusiastic") {
    ...
    if (topReq) {
      paragraphs.push(
        `What excites me most about ${company} is the focus on ${topReq.toLowerCase().slice(0, 120)}. I'd love to bring my energy and skills to help drive that mission forward!`
      );
    } else {
      paragraphs.push(
    ...
    if (topReq) {
      paragraphs.push(
        `I am particularly drawn to ${company} and the emphasis on ${topReq.toLowerCase().slice(0, 120)}. I believe my skills and experience align well with the challenges ahead.`
      );
    } else {
      paragraphs.push(

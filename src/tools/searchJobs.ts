  ...
  try {
    ...
    if (!res.ok) {
      ...
      for (const job of listings) {
        ...
        if (!text.includes(keyword) && !keyword.split(/\s+/).some((w: string) => text.includes(w))) continue;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        while ((match = itemRegex.exec(xml)) !== null && jobs.length < max_results) {
          ...
          if (!keyword.split(/\s+/).some((w) => text.includes(w))) continue;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        for (const job of data) {
          ...
          const text = `${job.title || ""} ${job.company_name || ""} ${(job.tags || []).map((t: any) => t.name || t).join(" ")} ${job.description || ""}`.toLowerCase();
          if (!keyword.split(/\s+/).some((w) => text.includes(w))) continue;

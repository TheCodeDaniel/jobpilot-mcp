...
export async function searchJobs(args: {
  ...
  try {
    ...
    const res = await fetch(
      `https://himalayas.app/api/jobs?q=${q}&limit=20&remote=true`,
      { headers: { "User-Agent": "JobPilot-MCP/1.0" } }
    );
    ...
    if (res.ok) {
      ...
      for (const job of jobs) {
        if (!job.title && !job.name) continue;
        ...
        allFetchedJobs.push({
          ...
          tags: Array.isArray(job.tags) ? job.tags.map((t: any) => typeof t === "string" ? t : t.name || "") : [],
